...
/**
 * This Composite is for selecting the printer in the Touch Screen preference.
	...
	private void createContents() {		
		if (tableDescriptor != null){			
			...
			for (int i=0;i < listOfColumns.size();i++) {
				if(Util.isSmallerResolution()) {
					listOfColumns.get(i).getTableColumn().setResizable(true);
				} else{
			...
			btnPrevious.setImage(new Image(Display.getCurrent(), getClass().getResourceAsStream(IImageConstants.DISPLAY_PREVIOUS_ARROW)));
			...
			btnNext.setImage(new Image(Display.getCurrent(), getClass().getResourceAsStream(IImageConstants.DISPLAY_NEXT_ARROW)));
			...
			if(Util.isSmallerResolution()) {
				gridData2.heightHint = 34;
				gridData2.widthHint = 40;
				gridData11.heightHint = 34;
				gridData11.widthHint = 40;
			}
//			lblPrefLogOutMsg = new SDSTSLabel(this, SWT.NONE, LabelLoader.getLabelValue("Voucher printer preferences value can be set only when logged off."));
		}
	}
